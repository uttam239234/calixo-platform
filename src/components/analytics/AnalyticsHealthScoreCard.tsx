...
import type { AnalyticsHealthScore } from "@/core/analytics";
...
interface AnalyticsHealthScoreCardProps {
  health: AnalyticsHealthScore | null;
  loading?: boolean;
}

export default function AnalyticsHealthScoreCard({ health, loading = false }: AnalyticsHealthScoreCardProps) {
  if (loading || !health) {
    return (
      <Card>
        <CardHeader title="Health Score" description="Weighted analytics health" />
        <CardContent>
  ...
  return (
    <Card>
      <CardHeader title="Health Score" description="Weighted across revenue, channels, funnel, forecast, and connectors" />
      <CardContent>
