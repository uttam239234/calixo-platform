...
export function AdsHealthScoreCard() {
  const { healthScore } = useCampaigns();
  const circumference = 2 * Math.PI * 26;
  const offset = circumference * (1 - healthScore.score / 100);

  return (
    <Card>
      <CardHeader title="Ads Health Score" description="Weighted across budget pacing, campaign performance, connectors, creative, and backlog" />
      <CardContent>
          ...
          <div className="relative h-16 w-16 flex-shrink-0">
            <svg viewBox="0 0 60 60" className="h-16 w-16 -rotate-90">
              ...
              <circle
                ...
                className={ringColor(healthScore.score)}
              />
            </svg>
