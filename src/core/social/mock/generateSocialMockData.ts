import type { SocialAccount, SocialPlatform, SocialPost, SocialRecommendation } from "../types";

/**
 * Verbatim-migrated from `features/social/mock-data.ts` — today's account numbers are the single
 * source of truth every other view (Analytics' platform metrics, Competitors' "Your Brand" row)
 * now derives from, instead of each inventing its own independent total. See `SocialEngine`'s
 * `computeSocialOverview()`/`computeSocialPlatformSummaries()` for the live aggregation.
 */
const ACCOUNT_SEED: Omit<SocialAccount, "organizationId" | "isLiveConnector">[] = [
...
const POST_SEED: Omit<SocialPost, "organizationId">[] = [
...
export function generateSocialAccounts(organizationId: string): SocialAccount[] {
  return ACCOUNT_SEED.map(account => ({ ...account, organizationId, isLiveConnector: false }));
}

export function generateSocialPosts(organizationId: string): SocialPost[] {
  return POST_SEED.map(post => ({ ...post, organizationId }));
}

export const SOCIAL_RECOMMENDATIONS_SEED: SocialRecommendation[] = [
  { id: "rec-1", title: "Post Instagram carousel at 7:30 PM", description: "Your audience is 32% more active during this window on Thursdays.", impact: "High", category: "Timing", status: "new" },
  { id: "rec-2", title: "Repurpose the AI report for LinkedIn", description: "A five-slide document post could extend reach by an estimated 18K impressions.", impact: "High", category: "Content", status: "new" },
  { id: "rec-3", title: "Reply to 24 high-intent comments", description: "These conversations contain product and pricing questions worth prioritizing.", impact: "Medium", category: "Engagement", status: "new" },
  { id: "rec-4", title: "Reconnect YouTube", description: "Restore the channel connection to resume publishing and performance tracking.", impact: "Medium", category: "Growth", status: "new" },
];

export function createPostId(): string {
  return typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : `social-${Date.now()}-${Math.random().toString(36).slice(2)}`;
}

export const SOCIAL_PLATFORM_ALIAS: Record<SocialPlatform, SocialPlatform> = {
  Facebook: "Facebook",
  Instagram: "Instagram",
  LinkedIn: "LinkedIn",
  X: "X",
